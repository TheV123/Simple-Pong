...
#This program creates a simple Pong game using Python Turtle graphics with an AI bot (that's very hard to beat)
...
while True:
    if not is_paused:
        ...
        #Ai player
        if p2.ycor() < ball.ycor() and abs(p2.ycor() - ball.ycor()) > 10:
            y =p2.ycor() 
            y += 20
            p2.sety(y)
        elif p2.ycor() > ball.ycor() and abs(p2.ycor() - ball.ycor()) > 10:
            y =p2.ycor() 
            y -= 20
            p2.sety(y)
    else:
